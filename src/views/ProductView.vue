  ...
  <section class="container flex flex-col gap-4 mx-auto text-white my-10 px-4">
    ...
    >
      <div class="flex flex-col gap-4 justify-between h-1/2">
        ...
        />
        <div class="flex flex-row w-full h-1/4 justify-center items-center gap-4">
          <div v-for="(image, i) in product.images" :key="i" class="w-1/2">
            <div
              class="flex flex-row overflow-hidden rounded-lg cursor-pointer"
              :class="currentIndex === i ? 'opacity-100' : 'opacity-30'"
              @click.prevent="currentIndex = i"
            >
              <img :src="image" :alt="image" class="h-full w-full object-cover object-center" />
            </div>
          </div>
        </div>
      </div>
      <div class="flex flex-col gap-6 justify-between mb-6 md:mb-2">
        <div class="flex flex-col gap-2">
          <h2 class="font-extrabold text-2xl lg:text-3xl tracking-wide">
            {{ product.title }}
          </h2>
          <p class="text-md text-primary xl:text-2xl">{{ product.category.name }}</p>
        </div>
        ...
        <div class="flex flex-row items-end justify-between">
          ...
          <a
            href="#"
            class="font-bold xl:text-xl py-2 2xl:py-4 px-4 2xl:px-8 rounded-lg bg-primary lg:mr-10"
            >Add to cart</a
          >
        </div>
      </div>
